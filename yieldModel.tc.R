...
d.F.plot <- left_join(d.F.plot,d.F %>% select(-ID,-Shrub.kg,-Tot.fruits,-fruitset,-propCBB,-propCBD,-fruit.drop,-Tot.leaves,-leaf.drop,-prop.ldrop,-prop.fdrop,-propLM,-propCLR,-propWilt,-propHerb,-Shrub.kg.1,-low.yield),by=c("Plot","year"))
...
#2014
fm<-lm(Shrub.kg~rescale(BA.legume) + rescale(labour) +  rescale(GapDry)+rescale(Shannon.i)+ rescale(Tot.P.ppm) + rescale(CN.ratio) +
...
fm14<-lm(Shrub.kg~rescale(BA.legume) + rescale(labour) + rescale(Shannon.i)+rescale(GapDry) +rescale(CN.ratio) +
           rescale(propCBD) + rescale(propCLR) + rescale(propCBB) + rescale(coffee.area.ha) +
           rescale(tmax.anom.fruit)+rescale(elevation) + rescale(patcharea),data=d.F.new.14)
summary(fm14)
...
#2015
d.F.new15 <- d.F.new1 %>% filter(year==2015)
...
#2016
d.F.new16 <- d.F.new1 %>% filter(year==2016)
...
diagnos15 <- data.frame(Resid = resid(dm15, type = "pearson"), Fitted = fitted(dm15),Variable = d.F.new15)
...
diagnos16 <- data.frame(Resid = resid(dm16, type = "pearson"), Fitted = fitted(dm16),Variable = d.F.new16)
...
#picked delta of 2 because delta of 6 was >60 models
dredg.m14<-subset(fm14.d,delta<2)
write.csv(dredg.m14,paste0(getwd(),"/Analysis/ES/Yld.2014_dredged01.csv"))

#2015&2016
dm.d<-dredge(dm)

#picked delta of 2 because delta of 6 was >200 models
...
cand.set.14[[1]]<-lm(Shrub.kg~rescale(GapDry) + rescale(Shannon.i) + rescale(tmax.anom.fruit) +
                       rescale(elevation),data=d.F.new.14)
cand.set.14[[2]]<-lm(Shrub.kg~rescale(CN.ratio) + rescale(Shannon.i) + rescale(tmax.anom.fruit) +
                       rescale(elevation),data=d.F.new.14)
cand.set.14[[3]]<-lm(Shrub.kg~rescale(Shannon.i) + rescale(tmax.anom.fruit) +
                       rescale(elevation),data=d.F.new.14)
cand.set.14[[4]]<-lm(Shrub.kg~rescale(BA.legume) + rescale(Shannon.i) + rescale(patcharea) +
                       rescale(elevation),data=d.F.new.14)
cand.set.14[[5]]<-lm(Shrub.kg~rescale(Shannon.i) + rescale(patcharea) +
                       rescale(elevation),data=d.F.new.14)
...
cand.set.15<-list()
#delta 2 has 14 models reduced to 4
cand.set.15[[1]]<-lm(diff.yld~rescale(BA.legume) + rescale(coffee.area.ha) + rescale(low.yield) +
                      rescale(elevation)*rescale(patcharea),data=d.F.new15)
cand.set.15[[2]]<-lm(diff.yld~rescale(coffee.area.ha) + rescale(low.yield) +
                    rescale(elevation)*rescale(patcharea),data=d.F.new15)
cand.set.15[[3]]<-lm(diff.yld~rescale(coffee.area.ha) + rescale(low.yield) +
                      rescale(patcharea),data=d.F.new15)
#include elevation without interaction term for model averaging
cand.set.15[[4]]<-lm(diff.yld~rescale(coffee.area.ha) + rescale(low.yield) +
                       rescale(elevation),data=d.F.new15)

#for 2016 yield difference
cand.set.16<-list()
#delta 2 has 14 models reduced to 4
cand.set.16[[1]]<-lm(diff.yld~rescale(fruitset) + rescale(coffee.area.ha) + rescale(low.yield) +
                       rescale(tmax.fruit)+rescale(patcharea),data=d.F.new16)
cand.set.16[[2]]<-lm(diff.yld~rescale(fruitset) + rescale(coffee.area.ha) + rescale(low.yield) +
                       rescale(propCLR)+rescale(patcharea),data=d.F.new16)
cand.set.16[[3]]<-lm(diff.yld~rescale(fruitset) + rescale(elevation) + rescale(low.yield)
                       ,data=d.F.new16)
cand.set.16[[4]]<-lm(diff.yld~rescale(fruitset) + rescale(coffee.area.ha) + rescale(low.yield) +
                       rescale(tmax.fruit),data=d.F.new16)
cand.set.16[[5]]<-lm(diff.yld~rescale(coffee.area.ha) + rescale(low.yield) +
                       rescale(propCLR)+rescale(patcharea),data=d.F.new16)
cand.set.16[[6]]<-lm(diff.yld~rescale(coffee.area.ha) + rescale(low.yield) +
                       rescale(tmax.fruit)+rescale(patcharea),data=d.F.new16)
...
write.csv(res.table,paste0(getwd(),"/Analysis/ES/AICtab_yld14.delta2.csv"))
...
write.csv(res.table,paste0(getwd(),"/Analysis/ES/AICtab_ylddiff15.delta2.csv"))

res.table <-aictab(cand.set = cand.set.16, modnames = Modnames.16, sort = TRUE)
write.csv(res.table,paste0(getwd(),"/Analysis/ES/AICtab_ylddiff16.delta2.csv"))

#2014
topmodels.avg<-model.avg(cand.set.14) 
sink(paste0(getwd(),"/Analysis/ES/Model.Average_yld14.delta2.txt"))
...
vars<-list()
for(i in 1:nrow(x14)){
  vars[[i]]<-data.frame(cbind(x14$Comparison[i],modavg(cand.set.14,x14$Comparison[i],uncond.se = "revised",modnames = Modnames.14)$Mod.avg.beta,modavg(cand.set.14,x14$Comparison[i],uncond.se = "revised",modnames = Modnames.14)$Uncond.SE,modavg(cand.set.14,x14$Comparison[i],uncond.se = "revised",modnames = Modnames.14)$Lower.CL,modavg(cand.set.14,x14$Comparison[i],uncond.se = "revised",modnames = Modnames.14)$Upper.CL),stringsAsFactors = F)
}
...
write.csv(tmp.14,paste0(getwd(),"/Analysis/ES/Model.Average_yld14.delta2.confint.csv"))
...
tmp.14$Comparison<-factor(tmp.14$Comparison,levels=tmp.14[order(tmp.14$Importance,decreasing=F),"Comparison"],
                       labels=c("Basal Area of\nLeguminous Trees","Soil C:N","Patch Area","CanopyGap","Maximum Temperature\nAnomaly","Shade Diversity","Elevation","(Intercept)"))
...
sink(paste0(getwd(),"/Analysis/ES/Model.Average_ylddiff15.delta2.txt"))
...
write.csv(tmp.15,paste0(getwd(),"/Analysis/ES/Model.Average_ylddiff15.delta2.confint.csv"))
...
tmp.15$Comparison<-factor(tmp.15$Comparison,levels=tmp.15[order(tmp.15$Importance,decreasing=F),"Comparison"],
                       labels=c("Basal Area of\nLeguminous Trees","Elevation:Patch Area","Elevation","Patch Area","Coffee Land Area","Low Yielding Plot","(Intercept)"))
...
sink(paste0(getwd(),"/Analysis/ES/Model.Average_ylddiff16.delta2.txt"))
...
vars<-list()
for(i in 1:nrow(x16)){
  vars[[i]]<-data.frame(cbind(x16$Comparison[i],modavg(cand.set.16,x16$Comparison[i],uncond.se = "revised",modnames = Modnames.16)$Mod.avg.beta,modavg(cand.set.16,x16$Comparison[i],uncond.se = "revised",modnames = Modnames.16)$Uncond.SE,modavg(cand.set.16,x16$Comparison[i],uncond.se = "revised",modnames = Modnames.16)$Lower.CL,modavg(cand.set.16,x16$Comparison[i],uncond.se = "revised",modnames = Modnames.16)$Upper.CL),stringsAsFactors = F)
}
...
write.csv(tmp.16,paste0(getwd(),"/Analysis/ES/Model.Average_ylddiff16.delta2.confint.csv"))
...
tmp.16$Comparison<-factor(tmp.16$Comparison,levels=tmp.16[order(tmp.16$Importance,decreasing=F),"Comparison"],
                          labels=c("Elevation","Incidence of CLR","Maximum Temperature\nDuring Fruiting","Patch Area","Fruitset","Coffee Land Area","Low Yielding Plot","(Intercept)"))
...
df<-d.F.new.14 %>% group_by(Plot,year) %>% 
  mutate(Shrub.kg.mod=tmp.14[tmp.14$Comparison=="(Intercept)","Estimate"]+tmp.14[tmp.14$Comparison=="rescale(GapDry)","Estimate"]*z.GapDry+tmp.14[tmp.14$Comparison=="rescale(Shannon.i)","Estimate"]*z.Shannon.i+tmp.14[tmp.14$Comparison=="rescale(CN.ratio)","Estimate"]*z.CN.ratio+
         ...
         tmp.14[tmp.14$Comparison=="rescale(tmax.anom.fruit)","Estimate"]*z.tmax.anom.fruit,low.yield1=1) %>% mutate(low.yield1=replace(low.yield1,low.yield<0.5,0))
...
df.15<-d.F.new15 %>% group_by(Plot,year) %>% mutate(diff.yld.mod=tmp.15[tmp.15$Comparison=="(Intercept)","Estimate"]+tmp.15[tmp.15$Comparison=="rescale(coffee.area.ha)","Estimate"]*z.coffee.area.ha+tmp.15[tmp.15$Comparison=="rescale(low.yield)","Estimate"]*z.low.yield+
                                                  tmp.15[tmp.15$Comparison=="rescale(elevation)","Estimate"]*z.elevation + tmp.15[tmp.15$Comparison=="rescale(patcharea)","Estimate"]*z.patcharea + tmp.15[tmp.15$Comparison=="rescale(BA.legume)","Estimate"]*z.BA.legume+
                                                  tmp.15[tmp.15$Comparison=="rescale(elevation):rescale(patcharea)","Estimate"]*z.elevation*z.patcharea,low.yield1=1) %>% mutate(low.yield1=replace(low.yield1,low.yield<0.5,0))

ggplot(df.15,aes(diff.yld,diff.yld.mod)) + geom_point(aes(color=factor(low.yield1))) + geom_abline(slope=1,intercept=0,linetype="dashed") +
  ylim(-0.6,0.5)+xlim(-0.6,0.5)+
  xlab("Observed Difference in Yield [kg]")+ylab("Modelled Difference in Yield [kg]")+
...
d.F.new16$z.low.yield=rescale(d.F.new16$low.yield)
...
df.16<-d.F.new16 %>% group_by(Plot,year) %>% mutate(diff.yld.mod=tmp.16[tmp.16$Comparison=="(Intercept)","Estimate"]+tmp.16[tmp.16$Comparison=="rescale(coffee.area.ha)","Estimate"]*z.coffee.area.ha+tmp.16[tmp.16$Comparison=="rescale(low.yield)","Estimate"]*z.low.yield+
                                                      tmp.16[tmp.16$Comparison=="rescale(elevation)","Estimate"]*z.elevation + tmp.16[tmp.16$Comparison=="rescale(patcharea)","Estimate"]*z.patcharea + tmp.16[tmp.16$Comparison=="rescale(propCLR)","Estimate"]*z.propCLR+
                                                      tmp.16[tmp.16$Comparison=="rescale(fruitset)","Estimate"]*z.fruitset + tmp.16[tmp.16$Comparison=="rescale(tmax.fruit)","Estimate"]*z.tmax.fruit,low.yield1=1) %>% mutate(low.yield1=replace(low.yield1,low.yield<0.6,0))

ggplot(df.16,aes(diff.yld,diff.yld.mod)) + geom_point(aes(color=factor(low.yield1))) + geom_abline(slope=1,intercept=0,linetype="dashed") +
  ylim(-0.6,0.5)+xlim(-0.6,0.5)+
  xlab("Observed Difference in Yield [kg]")+ylab("Modelled Difference in Yield [kg]")+
...
d.F.new1<-left_join(d.F.new1,d.F.tmax,by="Plot")
d.F.new1 <- d.F.new1 %>% group_by(Plot,year) %>% mutate(tmax.flower.diff=tmax.flower-tmax.flower.14,tmax.fruit.diff=tmax.fruit-tmax.fruit.14,low.yield1=1) %>%
  mutate(low.yield1=replace(low.yield1,low.yield<0.5,0))

ggplot(d.F.new1,aes(tmax.fruit.diff,diff.yld,group=year)) + geom_point() + facet_wrap(~year,ncol=2)

#3D figure of patcharea area and elevation
elev<-seq(as.integer(min(d.F.new1$elevation)),as.integer(max(d.F.new1$elevation)),by=1)
patch<-seq(as.integer(min(d.F.new1$patcharea)),as.integer(max(d.F.new1$patcharea)),by=5)
z.elev<-attributes(scale(d.F.new1$elevation))
z.patch<-attributes(scale(d.F.new1$patcharea))

z<-data.frame()
for(i in 1:length(elev)){
  for(j in 1:length(patch)){
    z[i,j] <- tmp[tmp$Comparison=="rescale(elevation)","Estimate"]*(elev[i]-z.elev[[2]])/(2*z.elev[[3]]) + tmp[tmp$Comparison=="rescale(patcharea)","Estimate"]*(patch[j]-z.patch[[2]])/(2*z.patch[[3]]) +
      tmp[tmp$Comparison=="rescale(elevation):rescale(patcharea)","Estimate"]*(elev[i]-z.elev[[2]])/(2*z.elev[[3]])*(patch[j]-z.patch[[2]])/(2*z.patch[[3]])
  }
}
z1<-as.matrix(z)
x <-patch
...
z_g<-gather(z,key="patch",value="yld_diff",-elevation)

ggplot(z_g, aes( as.numeric(patch), elevation, z = yld_diff)) +geom_raster(aes(fill=yld_diff)) +
  scale_fill_gradientn(colours = terrain.colors(20)) + theme_classic() + ylab("Elevation [m]") + xlab("Patch Area [ha]")+
  labs(fill="Shrub Yield\nDifference") + ggtitle("Interaction of Elevation and Patch Area for 2015 and 2016") + theme(text=element_text(size=14))
ggsave(paste0(getwd(),"/Analysis/ES/Modelled.ylddiff.elev.vs.patcharea.pdf"),width=8,height=7)
...
g1<-ggplot(d.F.new1 ,aes(coffee.area.ha,pdiff.yld)) + geom_point(aes(color=factor(low.yield1))) + theme_classic() +
  xlab("Coffee Area [ha]") + ylab("Difference in Yield [%]") + labs(color="Low Yielding Plot")
g2<-ggplot(d.F.new1 ,aes(coffee.area.ha,diff.yld)) + geom_point(aes(color=factor(low.yield1))) + stat_smooth(method="lm") +
  theme_classic() + xlab("Coffee Area [ha]") + ylab("Difference in Yield [kg]") + labs(color="Low Yielding Plot")
g3<-ggplot(d.F.new1,aes(coffee.area.ha,low.yield)) + geom_point(aes(color=factor(low.yield1))) + stat_smooth(method="lm") +
