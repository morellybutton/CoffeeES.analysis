...
clim <- read_csv(paste0(getwd(),"/Analysis/ElNino/TerraClim_tmax.csv"), guess_max = 1e5) %>%
  ...
  gather(key="site",value="tmax",-Date,-year,-month) %>% mutate(tmax=tmax/10) %>% 
  group_by(site) %>% 
  arrange(Date) %>% 
...
vpd <- read_csv(paste0(getwd(),"/Analysis/ElNino/TerraClim_vpd.csv")) %>% 
  mutate(Date=as.Date(Date,format="%d/%m/%Y")) %>% mutate(year=year(Date),month=month(Date)) %>%
  gather(key="site",value="vpd",-Date,-year,-month) %>% mutate(vpd=vpd/100)

tmp <- vpd %>% filter(year>=1986 & year <= 2017) %>% 
  group_by(site,month) %>% 
  summarize(u_vpd = mean(vpd, na.rm=T)) #to get kPa
vpd <- left_join(vpd,tmp,by=c("site","month"))

clim <- left_join(clim,vpd, by=c("site","Date","month","year"))
...
ppt <- read_csv(paste0(getwd(),"/Analysis/ElNino/TerraClim_precip.csv")) %>% 
  mutate(Date=as.Date(Date,format="%d/%m/%Y")) %>% mutate(year=year(Date),month=month(Date)) %>%
  gather(key="site",value="ppt",-Date,-year,-month)
tmp <- ppt %>% filter(year>=1986 & year <= 2017) %>% 
  group_by(site,month) %>% 
  summarize(u_precip = mean(ppt, na.rm=T))
ppt <- left_join(ppt,tmp,by=c("site","month"))

clim <- left_join(clim,ppt, by=c("site","Date","month","year"))
