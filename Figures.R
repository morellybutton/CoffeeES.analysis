...
ggsave("/users/alex/Documents/Research/Africa/ECOLIMITS/Pubs/Coffee_ES/ShrubYield_Comparison.pdf",height=4,width=10)
...
tmp.15$Comparison<-factor(tmp.15$Comparison,levels=tmp.15[order(tmp.15$Importance,decreasing=F),"Comparison"],
                          labels=c("Maximum\nTemperature (Fruiting)","BA Legume:\nShade Diversity","Basal Area\nof Leguminous\nTrees","Elevation:Patch Area","Shade Diversity","Coffee Land\nArea","Elevation","Patch Area","(Intercept)"))
...
#add significance column
tmp.15$sig<-0
tmp.15<-tmp.15 %>% mutate(sig=replace(sig,Comparison=="Basal Area\nof Leguminous\nTrees"|Comparison=="Shade Diversity",1))
...
tmp.16$Comparison<-factor(tmp.16$Comparison,levels=tmp.16[order(tmp.16$Importance,decreasing=F),"Comparison"],
                          labels=c("Low Yielding Plot","Fruitset","BA Legume:\nShade Diversity","Maximum\nTemperature (Fruiting)","Basal Area\nof Leguminous\nTrees","Elevation:Patch Area","Shade Diversity","Elevation","Coffee Land Area","Patch Area","(Intercept)"))
...
library(arm)
tm<-glm(low.yield.bin~patcharea*elevation,data=d.F.new14,family="binomial")
xy<-summary(tm)

#shade diversity and leguminous trees do not predict low yielding farms
sm<-glm(low.yield.bin~rescale(Shannon.i)*rescale(BA.legume),data=d.F.new14,family="binomial")
summary(sm)

#coffee land area predict low yielding farms
clm<-glm(low.yield.bin~rescale(coffee.area.ha),data=d.F.new14,family="binomial")
summary(clm)
...
t1<-ggplot(z_g.ly, aes( as.numeric(patch), elevation, z = lowyld)) +geom_raster(aes(fill=lowyld)) +
  ...
  labs(fill="Probability") + ggtitle("Predicting Low Yielding Farms") + theme(text=element_text(size=14))

#do for 2014 - not significant
tm.14<-glm(low.yield14.bin~rescale(elevation),data=d.F.new14,family="binomial")
summary(tm.14)

#for 2015 - not significant
tm.15<-glm(low.yield15.bin~rescale(patcharea)+rescale(elevation),data=d.F.new15,family="binomial")
summary(tm.15)

#for 2016 - not significant
tm.16<-glm(low.yield16.bin~rescale(patcharea)+rescale(elevation),data=d.F.new16,family="binomial")
summary(tm.16)

ggsave("/users/alex/Documents/Research/Africa/ECOLIMITS/Pubs/Coffee_ES/Modelled.lowyield.elev.vs.patcharea.pdf",t1,width=5,height=4)
