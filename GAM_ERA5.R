...
era5<-transform(era5, Time = as.numeric(Date))
#ylab <- expression(Temperature ~ (degree*C))

#Start with Temperature
dF<-era5  %>% filter(Plot=="B13"&!is.na(tmax_anom_sigma_3mo))
...
## AR(2)
m5 <- gamm(precip_anom_sigma_3mo  ~s(Time),
           data = dF, correlation = corARMA(form = ~ 1|year, p = 3))


anova(m3$lme, m4$lme, m5$lme)
...
ggsave("/users/alex/Documents/Research/Africa/ECOLIMITS/Pubs/ElNino/Coffee_ES/Landscape/Era5.StdAnom.GAM.Comparison.pdf",height=9,width=12)
