...
d.F <- d.F %>% group_by(Plot,Shrub.id,year) %>% mutate(prop.fdrop = fruit.drop/Tot.fruits) %>% mutate(prop.fdrop = replace(prop.fdrop,Tot.fruits==0,0))
...
am.14d <-lmer(log(Shrub.kg.1)~ rescale(elevation) + rescale(CN.ratio) + rescale(density) +  rescale(K.meq)  + rescale(propCBD) + rescale(propCLR) + rescale(propCBB)  + 
...
qqp(df.hi.15$Shrub.kg.1, "lnorm")

options(na.action = "na.omit")

am.15d <-lmer(log(Shrub.kg.1)~ rescale(fruitset) + rescale(ah.flower) + rescale(Tot.P.ppm) + rescale(density) +  rescale(K.meq)  + rescale(propCBD) + rescale(propCLR) + rescale(propCBB)  + 
                rescale(BA.legume) + rescale(GapDry) + rescale(Shannon.i) + rescale(b.ffer) + rescale(prop.ldrop)  + rescale(prop.fdrop) + (1|Plot), 
...
dredg.m03<-subset(am.15d.d,delta<2)
write.csv(dredg.m03,paste0(getwd(),"/Analysis/ES/GLM.lnorm.doraani.2015_dredged01.csv"))

pdf(paste0(getwd(),"/Analysis/ES/Shrub.2015.doraani.lnorm_residuals.pdf"),width=8,height=5)
bwplot(am.15d@frame$Plot~resid(am.15d))
dev.off()
#plot standardized residuals versus fitted values by plot,
#id identifies critical value (standarized residuals greater than 1-id/2 standard normal quantile in absolute value identified in plot)
diagnos <- data.frame(Resid = resid(am.15d, type = "pearson"), Fitted = fitted(am.15d),Variable = df.hi.15$Plot[!is.na(df.hi.15$propCLR)] )
pdf(paste0(getwd(),"/Analysis/ES/Shrub.2015.doraani.lnorm_ResidualvFittedValues_all.pdf"),width=8,height=8)
xyplot(Resid ~ Fitted, data = diagnos)
dev.off()

## separate by variable
pdf(paste0(getwd(),"/Analysis/ES/Shrub.2015.doraani.lnorm_ResidualvFittedValues.pdf"),width=8,height=8)
xyplot(Resid ~ Fitted | Variable, data = diagnos)
dev.off()
#Assumption 2: Random effects are normally distributed with mean zero and covariance matrix (not depending on the group) and are independent for different groups
#test for normality of residuals
## we could add a line to indicate 'normal' with a bit more work
pdf(paste0(getwd(),"/Analysis/ES/Shrub.2015.doraani.lnorm_qqplotResiduals_byplot.pdf"),width=8,height=8)
qqmath(~Resid | Variable, data = diagnos, distribution = qnorm, prepanel = prepanel.qqmathline,
       panel = function(x, ...) {
         panel.qqmathline(x, ...)
         panel.qqmath(x, ...)
...
cand.set<-list()
#delta 2 has eight models reduced to 3
cand.set[[1]]<-lmer(log(Shrub.kg.1)~ rescale(ah.flower) + rescale(b.ffer) + rescale(density) + rescale(K.meq) + (1|Plot), data = df.hi.15 ,REML=F)
cand.set[[2]]<-lmer(log(Shrub.kg.1)~ rescale(b.ffer) + rescale(density) + rescale(K.meq) + (1|Plot), data = df.hi.15  ,REML=F)
cand.set[[3]]<-lmer(log(Shrub.kg.1)~ rescale(density) + rescale(K.meq) + (1|Plot), data = df.hi.15  ,REML=F)
...
##generate AICc table
res.table <-aictab(cand.set = cand.set, modnames = Modnames, sort = TRUE)
write.csv(res.table,paste0(getwd(),"/Analysis/ES/AICtab_doraani.2015.lnorm.delta2.csv"))
