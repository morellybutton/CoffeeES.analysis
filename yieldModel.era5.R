...
#variables were removed in the following order: (N.B. pH was too closely correlated with elevation)
#1) coffee.area.ha, 2) propCBD, 3) C.pct, 4) elevation:patch area, 5) CN.ratio
fm14d<-lm(Shrub.kg~rescale(GapDry)+rescale(Shannon.i)*rescale(BA.legume)  +
           rescale(propCLR) + rescale(propCBB) + 
            rescale(tmax.anom.fruit)+rescale(elevation)+rescale(patcharea) ,data=d.F.new.14)
summary(fm14d)
...
#                       rescale(elevation),data=d.F.new.14)#include a model without interaction between BA.legume and Shannon.i with lowest possible delta AIC
#cand.set.14[[4]]<-lm(Shrub.kg~rescale(Shannon.i) + rescale(GapDry) +
#                       rescale(elevation)+rescale(tmax.anom.fruit),data=d.F.new.14)#include a model without interaction between BA.legume and Shannon.i with lowest possible delta AIC
#cand.set.14[[5]]<-lm(Shrub.kg~rescale(Shannon.i) +
#                       rescale(elevation)+rescale(tmax.anom.fruit),data=d.F.new.14)
...
#                       rescale(elevation)+rescale(patcharea),data=d.F.new.14)
#cand.set.14[[8]]<-lm(Shrub.kg~rescale(Shannon.i) +
...
##generate AICc table
res.table <-AICcmodavg::aictab(cand.set = cand.set.14, modnames = Modnames.14, sort = TRUE)
write.csv(res.table,paste0(getwd(),"/Analysis/ES/AICtab_yld14.delta2.v4.csv"))
...
x14<-as.data.frame(cand.set.14[[1]]$coefficients)
colnames(x14)<-"Coefficients"
...
#write.csv(x14,paste0(getwd(),"/Analysis/ES/Model_yld14.delta2.v4.confint.csv"))
...
x15<-as.data.frame(summary(topmodels.avg.15)$coefmat.full[,5])
x15$Comparison<-rownames(x15)
colnames(x15)<-c("pvalue","Comparison")
...
tmp.15$Lower.CL<-as.numeric(confint(topmodels.avg.15, full = T)[,1])
tmp.15$Upper.CL<-as.numeric(confint(topmodels.avg.15, full = T)[,2])
...
ggsave(paste0(getwd(),"/Analysis/ES/Model_averaged_results_combined.v4.pdf"),height=6,width=18)
ggsave("/users/alex/Documents/Research/Africa/ECOLIMITS/Pubs/ElNino/Coffee_ES/Landscape/AnalysisFigures/Model_averaged_results_combined.v4.pdf",height=6,width=18)
...
#model yield for 2014
d.F.new.14<-d.F.new %>% filter(year==2014)
d.F.new.14$Shrub.kg.pred<-predict(cand.set.14[[1]])
...
df<-d.F.new.14 %>% group_by(Plot) %>% 
  mutate(Shrub.kg.mod=tmp.14[tmp.14$Comparison=="(Intercept)","Coefficients"]+tmp.14[tmp.14$Comparison=="rescale(elevation)","Coefficients"]*z.elevation +
           tmp.14[tmp.14$Comparison=="rescale(BA.legume)","Coefficients"]*z.BA.legume +
           tmp.14[tmp.14$Comparison=="rescale(Shannon.i)","Coefficients"]*z.Shannon.i +
           tmp.14[tmp.14$Comparison=="rescale(patcharea)","Coefficients"]*z.patcharea +
           ...
           tmp.14[tmp.14$Comparison=="rescale(Shannon.i):rescale(BA.legume)","Coefficients"]*z.BA.legume*z.Shannon.i)
...
#ERA5 Figures
#library(mgcv)
era5<-read_csv(paste0(getwd(),"/Analysis/ElNino/era5_anomalies.csv"))
...
#take averages of anomalies for each year
dF <- era5 %>% filter(Plot=="B13"&year>=2014&year<2017)
dF_summ <- dF %>% group_by(year) %>% summarise(m_tmax=mean(tmax_anom,na.rm=T),m_precip=mean(precip_anom,na.rm=T))
tm1<-aov(tmax_anom~factor(year),data=dF)
summary(tm1)
TukeyHSD(tm1)

g1<-ggplot(era5 %>% filter(Plot=="B13"&year>=2014&year<2017),aes(Date,precip_anom)) + geom_bar(stat="identity") + theme_classic() +
  ...
  theme(text=element_text(size=16)) + #annotate("text",x=as.Date("2015-04-01"),y=90,label="Hot & Dry Year",size=8,fontface =2) +
  annotate("text",x=as.Date("2014-04-01"),y=90,label="Normal Year",size=8,fontface =2) +
  annotate("text",x=as.Date("2014-04-01"),y=-60,label=paste0("Mean Anomaly = ",signif(dF_summ %>% filter(year==2014) %>% pull(m_precip),3)),size=6,fontface =2) +
  annotate("text",x=as.Date("2015-04-01"),y=-60,label=paste0(signif(dF_summ %>% filter(year==2015) %>% pull(m_precip),3)),size=6,fontface =2) +
  annotate("text",x=as.Date("2016-04-01"),y=-60,label=paste0(signif(dF_summ %>% filter(year==2016) %>% pull(m_precip),3)),size=6,fontface =2) #+
  #annotate("text",x=as.Date("2016-04-01"),y=90,label="Hotter & Drier Year",size=8,fontface =2) 
  #geom_segment(aes(x = as.Date("2016-01-01"), y = 70, xend = as.Date("2016-10-01"), yend = 70), size=0.5,arrow = arrow(length = unit(0.5, "cm")))

g3<-ggplot(era5 %>% filter(Plot=="B13"&year>=2014&year<2017),aes(Date,tmax_anom)) + geom_bar(stat="identity") + theme_classic() +
  geom_rect(inherit.aes = F,mapping=aes(xmin=as.Date("2015-06-01"),xmax=as.Date("2016-03-01"),ymin=-Inf,ymax=Inf),fill='lightgrey',alpha=1/50) +
  ylab("Maximum Temperature\nAnomaly [C]") + xlab("Date") + geom_vline(data=harvest,aes(xintercept=as.Date(harvest.date)),linetype="dashed")  +
  theme(text=element_text(size=16)) +
  #annotate("text",x=as.Date("2014-04-01"),y=2.5,label="Normal Year",size=8,fontface =2) +
  annotate("text",x=as.Date("2014-04-01"),y=-2.5,label=paste0("Mean Anomaly = ",signif(dF_summ %>% filter(year==2014) %>% pull(m_tmax),3)),size=6,fontface =2) +
  annotate("text",x=as.Date("2015-04-01"),y=-2.5,label=paste0(signif(dF_summ %>% filter(year==2015) %>% pull(m_tmax),3)),size=6,fontface =2) +
  annotate("text",x=as.Date("2016-04-01"),y=-2.5,label=paste0(signif(dF_summ %>% filter(year==2016) %>% pull(m_tmax),3)),size=6,fontface =2)
  #+ annotate("text",x=as.Date("2015-01-01"),y=2.5,label="Hot Year",size=8) +
...
ggsave("/users/alex/Documents/Research/Africa/ECOLIMITS/Pubs/ElNino/Coffee_ES/Landscape/Era5.Anom.Comparison.pdf",height=8,width=12)
